...
const promptSets = {
    ...
    Jetpack: [
        { text: "Hold a long 'Ahhh' to fly!", target: 'Jetpack' }
    ]
};
...
let jetpackFuel = 0;
let bubbles = [];
let starFly = { x: 0, y: -2000, active: false, size: 30 };
let sustainedTime = 0;
...
function executeJump(freq) {
    ...
    // --- LINEAR JUMP ENGINE ---
    // Map freq between [low] and [mega * 1.5] into a power scale of 0.0 to 1.0
    const settings = ageSettings[currentAge];
...
function resetGame() {
    ...
    if (currentSet === 'Jetpack') {
        ...
        starFly.active = true;
        starFly.x = canvas.width / 2;
        starFly.y = -1500; // High above
    } else {
        starFly.active = false;
    }
}
...
function update() {
    ...
    // Star Fly Collision
    if (starFly.active) {
        const dx = (frog.x + frog.width / 2) - starFly.x;
        const dy = (frog.y + frog.height / 2) - starFly.y;
        const dist = Math.sqrt(dx * dx + dy * dy);
        if (dist < 50) {
            spawnParticles(starFly.x, starFly.y, '#FFD700');
            starFly.active = false;
            score += 10;
            scoreEl.innerText = score;
            promptTextEl.innerText = "MISSION SUCCESS!";
            setTimeout(() => {
                resetGame();
                updatePrompt();
            }, 2000);
...
function draw() {
    ...
    if (starFly.active) {
        ...
        ctx.beginPath();
        const spikes = 5;
        const outerRadius = starFly.size;
        ...
        ctx.moveTo(starFly.x, starFly.y - outerRadius);
        for (let i = 0; i < spikes; i++) {
            x = starFly.x + Math.cos(rot) * outerRadius;
            y = starFly.y + Math.sin(rot) * outerRadius;
            ctx.lineTo(x, y);
            rot += step;

            x = starFly.x + Math.cos(rot) * innerRadius;
            y = starFly.y + Math.sin(rot) * innerRadius;
            ctx.lineTo(x, y);
            rot += step;
        }
        ctx.lineTo(starFly.x, starFly.y - outerRadius);
        ctx.closePath();
        ctx.fill();
        ctx.shadowBlur = 0;

        // Wings for the Star-Fly
        ctx.fillStyle = 'rgba(200, 230, 255, 0.6)';
        ctx.beginPath();
        ctx.ellipse(starFly.x - 25, starFly.y, 25, 10, -0.5 + Math.sin(Date.now() / 100) * 0.2, 0, Math.PI * 2);
        ctx.ellipse(starFly.x + 25, starFly.y, 25, 10, 0.5 - Math.sin(Date.now() / 100) * 0.2, 0, Math.PI * 2);
        ctx.fill();
    }
